...
import { 
  ...
  Menu, LogOut, Settings, Bell, Building, UserCircle
} from 'lucide-react';
...
const WorkerTopNavbar = ({ setIsMobileSidebarOpen, onLogout, userProfile }) => {
  ...
  useEffect(() => {
    setNotifications([
      { id: 1, title: t('notifications.sampleTaskAssignedTitle'), description: t('notifications.sampleTaskAssignedDesc'), time: t('notifications.time.minutesAgo', {count: 15}), read: false },
      { id: 2, title: t('notifications.samplePaymentProcessedTitle'), description: t('notifications.samplePaymentProcessedDesc', {date: '2025-05-10'}), time: t('notifications.time.hoursAgo', {count: 2}), read: false },
      { id: 3, title: t('notifications.sampleTimeLogApprovedTitle'), description: t('notifications.sampleTimeLogApprovedDesc', {date: '2025-05-15'}), time: t('notifications.time.daysAgo', {count: 1}), read: true },
    ]);
  }, [t, locale]);
    ...
    <header className="h-16 border-b bg-card flex items-center justify-between px-4 md:px-6 sticky top-0 z-30 shrink-0">
      <div className="flex items-center">
        ...
        <Link to="/worker/dashboard" className="flex items-center space-x-2">
          ...
          <h1 className="text-xl font-bold text-primary hidden sm:block">{t('appName')} {t('workerPortal.title')}</h1>
           <h1 className="text-xl font-bold text-primary sm:hidden">{t('workerPortal.shortTitle')}</h1>
        </Link>
      </div>
          ...
          <DropdownMenuContent className="w-56" align="end" forceMount>
            <DropdownMenuLabel className="font-normal">
              <div className="flex flex-col space-y-1">
                <p className="text-sm font-medium leading-none">{userProfile?.name || t('login.roleWorker')}</p>
                <p className="text-xs leading-none text-muted-foreground">{userProfile?.email || "worker@example.com"}</p>
              </div>
            </DropdownMenuLabel>
            <DropdownMenuSeparator />
            <DropdownMenuGroup>
              <DropdownMenuItem onClick={() => navigate('/worker/settings')}><Settings className={locale === 'ar' ? "ml-2 h-4 w-4" : "mr-2 h-4 w-4"} /><span>{t('userMenu.settings')}</span></DropdownMenuItem>
            </DropdownMenuGroup>
            <DropdownMenuSeparator />
...
export default WorkerTopNavbar;
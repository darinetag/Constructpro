...
import { FlaskRound, Menu, Settings, LogOut, Bell, UserCircle } from 'lucide-react';
...
const LabTopNavbar = ({ setIsMobileSidebarOpen, onLogout, userProfile }) => {
  ...
  useEffect(() => {
    setNotifications([
      { id: 1, title: t('notifications.sampleLabTestRequestTitle'), description: t('notifications.sampleLabTestRequestDesc'), time: t('notifications.time.minutesAgo', {count: 5}), read: false },
      { id: 2, title: t('notifications.sampleLabAppointmentConfirmedTitle'), description: t('notifications.sampleLabAppointmentConfirmedDesc'), time: t('notifications.time.hoursAgo', {count: 2}), read: true },
    ]);
  }, [t, locale]);
  ...
  return (
    <header className="h-16 border-b bg-card flex items-center justify-between px-4 md:px-6 sticky top-0 z-30 shrink-0">
      <div className="flex items-center">
        ...
        <Link to="/lab/dashboard" className="flex items-center space-x-2 md:hidden">
          <FlaskRound className="h-7 w-7 text-primary" />
          <h1 className="text-xl font-bold text-primary">{t('labPortal.shortTitle')}</h1>
        </Link>
      </div>
          ...
          <DropdownMenuContent className="w-56" align="end" forceMount>
            <DropdownMenuLabel className="font-normal">
              <div className="flex flex-col space-y-1">
                <p className="text-sm font-medium leading-none">{userProfile?.name || t('login.roleLabPersonnel')}</p>
                <p className="text-xs leading-none text-muted-foreground">{userProfile?.email || "lab@example.com"}</p>
              </div>
            </DropdownMenuLabel>
            <DropdownMenuSeparator />
            <DropdownMenuGroup>
              <DropdownMenuItem onClick={() => navigate('/lab/settings')}><Settings className={locale === 'ar' ? "ml-2 h-4 w-4" : "mr-2 h-4 w-4"} /><span>{t('userMenu.settings')}</span></DropdownMenuItem>
            </DropdownMenuGroup>
            <DropdownMenuSeparator />
